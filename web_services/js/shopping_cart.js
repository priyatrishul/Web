...
// Do not use the following two methods;  they are private to this class
    this.getCookieValues = function() {  // PRIVATE METHOD
        ...
        for(i=0; i < arr.length; i++) {
            ...
            if(pair[0] == undefined || pair[1] == undefined || pair[2] == undefined || pair[3] == undefined || pair[4] == undefined || pair[5] == undefined) continue;
            this.skuArray[i] = pair[0];
            this.qtyArray[i] = pair[1];
			this.descArray[i] = pair[2];
			this.priceArray[i] = pair[3];
			this.imageArray[i] = pair[4];
			this.onhandArray[i] = pair[5];
            }         
        }
    ...
    this.writeCookie = function() {  // PRIVATE METHOD
        ...
        for(i=0; i < this.skuArray.length; i++) 
            toWrite += this.skuArray[i] + "|" + this.qtyArray[i] +"|" + this.descArray[i] +"|" + this.priceArray[i] +"|" + this.imageArray[i] +"|" + this.onhandArray[i] + "||";
...
//////////////////////////////////////////////////////////////////////////            
    ...
    this.add = function(sku, quantity,description,price,image,onhand) {
        sku = $.trim(sku);
        ...
        if(this.skuArray[i] == sku) {        
			...
			this.descArray[i]=description;
			this.priceArray[i]=price;
			this.imageArray[i]=image;
			this.onhandArray[i]=onhand;
            found = true;            
            }
        if(!found) {       
            this.skuArray.push(sku);
            this.qtyArray.push(quantity);
			this.descArray.push(description);
			this.priceArray.push(price);
			this.imageArray.push(image);
			this.onhandArray.push(onhand);
            }
        this.writeCookie();         
    }
    ...
    this.delete = function(sku) {
        ...
        if(index != -1) {      
			...
			this.onhandArray.splice(index,1);
            }         
    ...
    this.getCartArray = function() {
        ...
        for(i=0; i < this.skuArray.length; i++) {
			...
			returnArray[i].push(this.descArray[i]);
            returnArray[i].push(this.priceArray[i]);
			returnArray[i].push(this.imageArray[i]);
			returnArray[i].push(this.onhandArray[i]);
